...
//*************************   PARTIES RESERVATION ***************************************** */
    ...
    public function resInsert($data){
        $date_resa      = $data['date_resa'];
        $dad_resa   = $data['dad_resa'];
        $daf_resa        = $data['daf_resa'];
        $commentaire        = $data['commentaire'];
        ...
        //insert data if there is no error            
        $query = "INSERT INTO reservations (date_resa, dad_resa, daf_resa, commentaire, moderation, annul_resa, id_client, id_bien) VALUES (:date_resa, :dad_resa, :daf_resa, :commentaire, :moderation, :annul_resa, :id_client, :id_bien)";
        $sql = $this->db->pdo->prepare($query);
        $sql->bindValue(':date_resa', $date_resa);
        $sql->bindValue(':dad_resa', $dad_resa);
        $sql->bindValue(':daf_resa', $daf_resa);
        ...
        if($result){
            $msg = "<div class='alert alert-success'>Votre réservation à était créer</div>";
            return $msg;
            header("location: reservations/resInsert.php");
        }
    }
    ...
    public function bienInsert($data){
        ...
        //insert data if there is no error            
        $query = "INSERT INTO biens (nom_bien, rue_bien, cop_bien, vil_bien, sup_bien, nb_couchage, nb_pieces, nb_chambres, descriptif, ref_bien, statut_bien, id_type_bien ) VALUES (:nom_bien, :rue_bien, :cop_bien, :vil_bien, :sup_bien, :nb_couchage, :nb_pieces, :nb_chambres, :descriptif, :ref_bien, :statut_bien, :id_type_bien)";
        $sql = $this->db->pdo->prepare($query);
        $sql->bindValue(':nom_bien', $nom_bien);
        $sql->bindValue(':rue_bien', $rue_bien);
        $sql->bindValue(':cop_bien', $cop_bien);
